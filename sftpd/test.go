...
type Synthetic1 struct {
	synthetic
}

var synthRoot1 = &synthDir{map[string]*synthFile{
	"foo1": &synthFile{bs: []byte("foo contents")},
	"bar1": &synthFile{bs: []byte("bar contents")},
}}

func (fs Synthetic1) OpenDir(path string) (sftpd.Dir, error) {
	d := synthDir{m: map[string]*synthFile{}}
	for k, v := range synthRoot1.m {
		d.m[k] = v
	}
	return &d, nil
}